...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    m.react('🔥');
    let str = `\`${name2}\` esta teniendo sexo con  \`${name}\``.trim();

    if (m.isGroup) {
        try {
            // Obtener el archivo JSON desde GitHub
            const response = await axios.get('https://raw.githubusercontent.com/Elpapiema/Adiciones-para-AlyaBot-RaphtaliaBot-/refs/heads/main/video_json/NSFW/sexo.json');
